 ...
 */
export function getEnabledOAuthProviders(): Array<
  typeof oauthProviders.google | typeof oauthProviders.microsoft
> {
  return authConfig.enabledProviders
    ...
    .filter(
      (
        p
      ): p is typeof oauthProviders.google | typeof oauthProviders.microsoft =>
        p !== null
    );
}
...
export function validatePassword(password: string): {
  ...
  if (password.length < authConfig.passwordMinLength) {
    errors.push(
      `Password must be at least ${authConfig.passwordMinLength} characters long`
    );
  }
  ...
  if (
    authConfig.passwordRequireSymbols &&
    !/[!@#$%^&*(),.?":{}|<>]/.test(password)
  ) {
