...
import { checkRefundEligibility, processRefund, getRefundHistory } from "@/lib/stripe";
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (authResult.user.user_metadata?.app_role !== "admin") {
      return createErrorResponse("FORBIDDEN", "Admin privileges required", null, 403);
    }
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!userId) {
      return createErrorResponse("BAD_REQUEST", "User ID parameter is required", null, 400);
    }
    ...
    if (action === "eligibility") {
      ...
      return createErrorResponse("BAD_REQUEST", "Invalid action parameter", null, 400);
    }
