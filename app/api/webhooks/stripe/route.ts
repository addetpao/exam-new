...
async function handleRefund(charge: Stripe.Charge) {
  ...
  console.log(
    `Refund processed for user ${user.id}: ${charge.amount_refunded} ${charge.currency}`
  );
}
...
async function handlePaymentSuccess(invoice: Stripe.Invoice) {
  ...
  console.log(
    `Payment succeeded for user ${user.id}: ${invoice.amount_paid} ${invoice.currency}`
  );
}
...
async function handlePaymentFailed(invoice: Stripe.Invoice) {
  ...
  console.log(
    `Payment failed for user ${user.id}: ${invoice.amount_due} ${invoice.currency}`
  );
}
...
async function handleCheckoutCompleted(session: Stripe.Checkout.Session) {
  console.log(
    `Checkout completed: ${session.id} for customer: ${session.customer}`
  );
  ...
  if (session.subscription) {
    ...
    console.log(
      `One-time payment checkout completed: ${session.payment_intent}`
    );
  }
}
